...
$.widget( "ui.datagrid", {
	...
	_create: function() {
		...
		if(this.options.columns == null || !this.options.columns.length){
	 	...
	 	var header = this._header = $("<div class='ui-widget ui-datagrid-header'><div class='ui-datagrid-header-content'><table><thead><tr></tr></thead></table></div></div>");
		this.element.append(header);
		var headerContent = this._headerContent = header.children(".ui-datagrid-header-content");
		
		var body = this._body = $("<div class='ui-widget ui-datagrid-body'><div class='ui-datagrid-body-content'></div></div>");
	...
	_renderHeaderCheckboxCell: function(){
		var self = this;
		this._header.find(".ui-datagrid-header-checkboxcell").remove();
		if(!this.options.singleSelect){
			var th = $("<th class='ui-datagrid-header-checkboxcell' style='width:20px'><input type='checkbox'/></th>");
			th.click(function(ev){
				self._onHeaderCheckboxCell_click(this, ev);
				ev.stopPropagation();
			});
			this._header.find("tr").append(th);
		}
	...
	sortBy: function(columnName, direction){
		...
		if(cells && cells.length){
			...
			this._trigger("sorted", null, cell);
		}
	},
	_renderHeight: function(){
		if(this.options.height){
			var headerHeight = this._header.height();
			if(this.options.height >= headerHeight){
				this._body.height(this.options.height - headerHeight);
			}
		}
	},
	...
	appendRow: function(data){
		...
		datarow.datarow({
			...
			selected: function(sender, args){
				self._onDatarow_selected(sender, args);
			}, 
			unselected: function(sender, args){
				self._onDatarow_unselected(sender, args);
			}
		});
	...
	_onDatarow_selected: function(sender, args){
		if(this._bodyContent.find(".ui-datagrid-selected").length == this._rows.length){
			this._header.find(".ui-datagrid-header-checkboxcell input").attr("checked", "checked");
		}
		this._trigger("selectedRow", null, sender);
	},
	_onDatarow_unselected: function(sender, args){
		this._header.find(".ui-datagrid-header-checkboxcell input").removeAttr("checked");
		this._trigger("unselectedRow", null, sender);
	},
	_onHeaderCheckboxCell_click: function(sender, args){
		var self = this;
	...
	getSelectedRow: function(){
		var selectRows = $.grep(this._rows, function(row){
			return row.datarow("getSelected");
		});
		return selectRows;
	},
...
$.widget( "ui.datarow",{
	...
	_create: function() {
		...
		this.element.addClass("ui-widget ui-datagrid-row")
	...
	_renderCheckboxCell: function(){
		var self = this;
		this.element.find(".ui-datagrid-checkboxcell").remove();
		if(this.options.showCheckboxCell){
			var td = $("<td class='ui-datagrid-checkboxcell' style='width:20px'><input type='checkbox'/></td>");
	...
	_onDataRow_click: function(sender, args){
		var self = this;
		this._trigger("click", null, this.options.data);
		this.select();
	},
	select: function(){
		if(this.options.showCheckboxCell){
			this.element.find(".ui-datagrid-checkboxcell input").attr("checked", "checked");
		}
		this.element.addClass("ui-datagrid-selected");
		this._selected = true;
		this._trigger("selected", null, this.options.data);
	},
	unselect: function(){
		this.element.removeClass("ui-datagrid-selected")
		 .find(".ui-datagrid-checkboxcell input").removeAttr("checked");
		this._selected = false;
		this._trigger("unselected", null, this.options.data);
	},
	...
	_setOption: function(key, value){
		...
		$.Widget.prototype._setOption.apply(self, arguments);
		switch(key){
			case "data": this.update(value);break;
